  ...
  return (
    <div>
      <h2 id="card-heading" data-cy="CardHeading">
        <Translate contentKey="trilloIntegrationApp.card.home.title">Cards</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="trilloIntegrationApp.card.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/card/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="trilloIntegrationApp.card.home.createLabel">Create new Card</Translate>
          </Link>
        </div>
      </h2>
        ...
        {cardList && cardList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="trilloIntegrationApp.card.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('isArchived')}>
                  <Translate contentKey="trilloIntegrationApp.card.isArchived">Is Archived</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('isArchived')} />
                </th>
                ...
                <th>
                  <Translate contentKey="trilloIntegrationApp.card.board">Board</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="trilloIntegrationApp.card.boardList">Board List</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {cardList.map((card, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{card.position}</td>
                  <td>{card.dueDate ? <TextFormat type="date" value={card.dueDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{card.isArchived ? 'true' : 'false'}</td>
                  <td>{card.trelloId}</td>
                  ...
                  <td>{card.board ? <Link to={`/board/${card.board.id}`}>{card.board.id}</Link> : ''}</td>
                  <td>{card.boardList ? <Link to={`/board-list/${card.boardList.id}`}>{card.boardList.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="trilloIntegrationApp.card.home.notFound">No Cards found</Translate>
            </div>
          )
        )}
      </div>
