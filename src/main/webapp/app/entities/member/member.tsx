  ...
  return (
    <div>
      <h2 id="member-heading" data-cy="MemberHeading">
        <Translate contentKey="trilloIntegrationApp.member.home.title">Members</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="trilloIntegrationApp.member.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/member/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="trilloIntegrationApp.member.home.createLabel">Create new Member</Translate>
          </Link>
        </div>
      </h2>
        ...
        {memberList && memberList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="trilloIntegrationApp.member.id">ID</Translate>{' '}
                ...
                <th>
                  <Translate contentKey="trilloIntegrationApp.member.cards">Cards</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {memberList.map((member, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{member.trelloId}</td>
                  <td>{member.name}</td>
                  <td>
                    {member.cards
                      ? member.cards.map((val, j) => (
                          <span key={j}>
                            <Link to={`/card/${val.id}`}>{val.id}</Link>
                            {j === member.cards.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="trilloIntegrationApp.member.home.notFound">No Members found</Translate>
            </div>
          )
        )}
      </div>
